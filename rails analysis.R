...
commit_comments <- "https://api.github.com/repos/rails/rails/comments?per_page=100&page=%d" %>>%
  sprintf(1:50) %>>%
  list.load("json") %>>%
  list.ungroup
...
commit_comments_rails <- vapply(commit_comments, "[[", "", "body")
date_of_comments <- vapply(commit_comments, "[[", "", "created_at")


#Converting to Data Frame
dates_df <- tibble(line= 1:5000, date = date_of_comments)

text_df <- tibble(line = 1:5000, text = commit_comments_rails)
...
ggplot(comment_sentiments, aes(index, sentiment)) +
  geom_col(show.legend = FALSE) + ylab(label = "Sentiment Score Agregatted by 10 Comments") + xlab(label = "Batched Comments by 10") + ggtitle(label = "Sentiment Analysis using Bing Lexicon on commit comments on Rails Repo ")
...
ggplot(comment_sentiments_with_date, aes(index, sentiment)) +
  geom_col(show.legend = FALSE) + ylab(label = "Sentiment Score Agregatted by Quarter") + xlab(label = "Batched Comments by Quarter") + ggtitle(label = "Sentiment Analysis using Bing Lexicon on commit comments on Rails Repo ")

# Weekly commit count for past 52 weeks

weekly_commit_count <- "https://api.github.com/repos/rails/rails/stats/participation" %>>%
  list.load("json") %>>%
  list.ungroup

total_weekly_commit_count <- weekly_commit_count[1:52]
plot(total_weekly_commit_count, type="l", main = "Total No. Of Commits in Rails in Past year", xlab = "Week Number from today back a year", ylab="No. of Commits")
